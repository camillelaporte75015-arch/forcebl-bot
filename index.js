...
const commands = {
  ...
  async forcebl(message, args) {
    ...
    if (args[0] === 'info') {
      ...
      const embed = new EmbedBuilder()
        .setColor(0xe74c3c)
        .setTitle(`<@${target.id}> est forcebl de ce serveur`)
        .setDescription(`**Raison :** ${entry.reason || 'aucune raison'}\n\n<@${target.id}> a été forcebl par <@${entry.byId}>\n${entry.date} — ${entry.time}`);
      return message.channel.send({ embeds: [embed] });
    }
    ...
    if (db.isHey(target.id))
      return message.channel.send({ embeds: [new EmbedBuilder().setColor(0xe74c3c).setDescription(`Impossible de forcebl <@${target.id}>, cette personne est ultra protégée par le propriétaire du bot.`)] });
    ...
    if (isProtected && !isTrustOrOwner)
      return message.channel.send({ embeds: [new EmbedBuilder().setColor(0xe74c3c).setDescription(`<@${target.id}> est protect par un trust, vous ne pouvez pas forcebl cette personne.`)] });
    ...
    if (isProtected && isTrustOrOwner) {
      ...
      extraMsg = `<@${target.id}> n'est plus protect et a été forcebl de ce serveur.`;
    }
    ...
    const embed = new EmbedBuilder().setColor(0xe74c3c)
      .setDescription(`<@${target.id}> est maintenant dans la forcebl, il a été banni de ce serveur.\n\n**Raison :** ${reason || 'aucune raison'}\n**Forcebl par :** <@${message.author.id}>`);
  ...
  async unforcebl(message, args) {
    ...
    const embed = new EmbedBuilder().setColor(0x2ecc71).setTitle('UnForceBlacklist')
      .setDescription(`<@${target.id}> a été retiré de la forcebl et débanni.`);
    await message.channel.send({ embeds: [embed] });
  ...
  async ban(message, args) {
    ...
    if (args[0] === 'info') {
      ...
      const embed = new EmbedBuilder().setColor(0xe67e22).setTitle(`<@${target.id}> est banni de ce serveur`)
        .setDescription(`**Raison :** ${entry.reason || 'aucune raison'}\n\n<@${target.id}> a été banni par <@${entry.byId}>\n${entry.date} — ${entry.time}`);
      return message.channel.send({ embeds: [embed] });
    }
    ...
    if (db.isProtect(target.id) && !isTrustOrOwner)
      return message.channel.send({ embeds: [new EmbedBuilder().setColor(0xe74c3c).setDescription(`<@${target.id}> est protect par un trust, vous ne pouvez pas bannir cette personne.`)] });
    ...
    const embed = new EmbedBuilder().setColor(0xe67e22)
      .setDescription(`<@${target.id}> est maintenant dans la banlist, il a été banni de ce serveur.\n\n**Raison :** ${reason || 'aucune raison'}\n**Banni par :** <@${message.author.id}>`);
    await message.channel.send({ embeds: [embed] });
  ...
  async unban(message, args) {
    ...
    const embed = new EmbedBuilder().setColor(0x2ecc71)
      .setDescription(`<@${target.id}> a été retiré de la banlist et débanni.`);
    await message.channel.send({ embeds: [embed] });
  ...
  async ow(message, args) {
    ...
    const { date, time } = getNow();
    const embed = new EmbedBuilder().setColor(0x3498db).setDescription(`<@${target.id}> peut maintenant utiliser toutes les commandes de ce bot.`);
    await message.channel.send({ embeds: [embed] });
  ...
  async unow(message, args) {
    ...
    const { date, time } = getNow();
    const embed = new EmbedBuilder().setColor(0x95a5a6).setDescription(`<@${target.id}> ne peut plus utiliser les commandes de ce bot.`);
    await message.channel.send({ embeds: [embed] });
  ...
  async trust(message, args) {
    ...
    const { date, time } = getNow();
    const embed = new EmbedBuilder().setColor(0x9b59b6).setDescription(`<@${target.id}> est maintenant trust.`);
    await message.channel.send({ embeds: [embed] });
  ...
  async untrust(message, args) {
    ...
    const { date, time } = getNow();
    const embed = new EmbedBuilder().setColor(0x95a5a6).setDescription(`<@${target.id}> n'est plus trust.`);
    await message.channel.send({ embeds: [embed] });
    const logEmbed = new EmbedBuilder().setColor(0x95a5a6).setTitle("Retrait d'un membre")
  ...
  async wl(message, args) {
    ...
    const { date, time } = getNow();
    const embed = new EmbedBuilder().setColor(0x1abc9c).setDescription(`<@${target.id}> est maintenant wl.`);
    await message.channel.send({ embeds: [embed] });
  ...
  async unwl(message, args) {
    ...
    const { date, time } = getNow();
    const embed = new EmbedBuilder().setColor(0x95a5a6).setDescription(`<@${target.id}> a été supprimé de la wl.`);
    await message.channel.send({ embeds: [embed] });
    const logEmbed = new EmbedBuilder().setColor(0x95a5a6).setTitle("Retrait d'un membre")
      .setDescription(`**Membre :** ${target.username} (${target.id})\n**Par :** ${message.author.username}\n**Retrait du rôle :** wl\n**Date :** ${date}\n**Heure :** ${time}`);
    await sendLog(message.guild, LOGS.rem_role, logEmbed);
  ...
  async protect(message, args) {
    ...
    const { date, time } = getNow();
    const embed = new EmbedBuilder().setColor(0xf39c12).setDescription(`<@${target.id}> est maintenant protect.`);
    await message.channel.send({ embeds: [embed] });
  ...
  async unprotect(message, args) {
    ...
    db.removeProtect(target.id);
    const { date, time } = getNow();
    const embed = new EmbedBuilder().setColor(0x95a5a6).setDescription(`<@${target.id}> n'est maintenant plus protect.`);
    await message.channel.send({ embeds: [embed] });
    const logEmbed = new EmbedBuilder().setColor(0x95a5a6).setTitle("Retrait d'un membre")
  ...
  async hey(message, args) {
    ...
    const embed = new EmbedBuilder().setColor(0xf1c40f).setDescription(`On dirait que <@${target.id}> est maintenant intouchable.`);
    await message.channel.send({ embeds: [embed] });
  },
  ...
  async unhey(message, args) {
    ...
    const embed = new EmbedBuilder().setColor(0x95a5a6).setDescription(`<@${target.id}> n'est plus ultra protégé par toi, elle est vulnérable comme les autres.`);
    await message.channel.send({ embeds: [embed] });
  },
  ...
  async limitban(message, args) {
    ...
    const embed = new EmbedBuilder()
      .setColor(0xe67e22)
      .setTitle('Limite de ban configurée')
  ...
  async limitforcebl(message, args) {
    ...
    const embed = new EmbedBuilder()
      .setColor(0xe74c3c)
      .setTitle('Limite de forcebl configurée')
  ...
  // ── &unban all ────────────────────────────────────────────────────────────
  async unbanAll(message) {
    if (!hasPermission(message.author.id, 'ow'))
      return message.channel.send({ embeds: [noPermEmbed()] });

    const list = db.getBanList();
    if (!list.length)
      return message.channel.send({ embeds: [new EmbedBuilder().setColor(0x95a5a6).setDescription('La banlist est déjà vide.')] });

    for (const entry of list) {
      try { await message.guild.members.unban(entry.userId); } catch {}
      db.removeBan(entry.userId);
    }

    const { date, time } = getNow();
    const embed = new EmbedBuilder()
      .setColor(0x2ecc71)
      .setDescription(`**${list.length}** utilisateurs ont été unban.`);
    await message.channel.send({ embeds: [embed] });

    const PAGE_SIZE = 10;
    const pages = [];
    for (let i = 0; i < list.length; i += PAGE_SIZE) {
      const chunk = list.slice(i, i + PAGE_SIZE);
      let desc = '**Logs unban all**\n\n';
      for (const e of chunk) desc += `${e.username} / ${e.userId}\n`;
      desc += `\n**Exécuteur :** <@${message.author.id}>\n**Date :** ${date}\n**Heure :** ${time}`;
      pages.push(desc);
    }
    await sendPaginatedLog(message.guild, LOGS.unban, pages, 0x2ecc71);
  },

  // ── &unforcebl all ────────────────────────────────────────────────────────
  async unforceblAll(message) {
    if (!hasPermission(message.author.id, 'ow'))
      return message.channel.send({ embeds: [noPermEmbed()] });

    const list = db.getForceblList();
    if (!list.length)
      return message.channel.send({ embeds: [new EmbedBuilder().setColor(0x95a5a6).setDescription('La forcebl list est déjà vide.')] });

    for (const entry of list) {
      try { await message.guild.members.unban(entry.userId); } catch {}
      db.removeForcebl(entry.userId);
    }

    const { date, time } = getNow();
    const embed = new EmbedBuilder()
      .setColor(0x2ecc71)
      .setDescription(`**${list.length}** utilisateurs ont été unforcebl.`);
    await message.channel.send({ embeds: [embed] });
    ...
    for (let i = 0; i < list.length; i += PAGE_SIZE) {
      const chunk = list.slice(i, i + PAGE_SIZE);
      let desc = '**Logs unforcebl all**\n\n';
      for (const e of chunk) desc += `${e.username} / ${e.userId}\n`;
      desc += `\n**Exécuteur :** <@${message.author.id}>\n**Date :** ${date}\n**Heure :** ${time}`;
      pages.push(desc);
    }
    await sendPaginatedLog(message.guild, LOGS.unforcebl, pages, 0x2ecc71);
  },
...
// ─── Paginated log sender with buttons ───────────────────────────────────────

const { ActionRowBuilder, ButtonBuilder, ButtonStyle } = require('discord.js');

async function sendPaginatedLog(guild, channelId, pages, color) {
  ...
  try {
    ...
    if (pages.length === 1) {
      await ch.send({ embeds: [new EmbedBuilder().setColor(color).setDescription(pages[0])] });
      return;
    }
    ...
    const getRow = (p) => new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('prev_log').setLabel('◀').setStyle(ButtonStyle.Secondary).setDisabled(p === 0),
      new ButtonBuilder().setCustomId('next_log').setLabel('▶').setStyle(ButtonStyle.Secondary).setDisabled(p === pages.length - 1),
    );

    const msg = await ch.send({ embeds: [getEmbed(0)], components: [getRow(0)] });
    const collector = msg.createMessageComponentCollector({ time: 120_000 });
    collector.on('collect', async (i) => {
      if (i.customId === 'prev_log') currentPage = Math.max(0, currentPage - 1);
      if (i.customId === 'next_log') currentPage = Math.min(pages.length - 1, currentPage + 1);
    ...
    console.error('Paginated log error:', e.message);
  }
}

// ─── Anti-unban externe ───────────────────────────────────────────────────────
// Si quelqu'un est débanni hors du bot (autre bot, admin...) on le rebanni immédiatement

client.on('guildBanRemove', async (ban) => {
  const userId = ban.user.id;
  const forceblEntry = db.getForceblEntry(userId);
  const banEntry = db.getBanEntry(userId);

  if (forceblEntry || banEntry) {
    try {
      await ban.guild.members.ban(userId, { reason: 'Reban automatique — unban externe non autorisé' });
    } catch (e) {
      console.error('Reban error:', e.message);
    }
  }
});
...
client.on('messageCreate', async (message) => {
  ...
  try {
    switch (cmd) {
      ...
      case 'unforcebl':
        if (args[0] === 'all') await commands.unforceblAll(message);
        else await commands.unforcebl(message, args);
        break;
      case 'ban':          await commands.ban(message, args); break;
      case 'unban':
        if (args[0] === 'all') await commands.unbanAll(message);
        else await commands.unban(message, args);
        break;
